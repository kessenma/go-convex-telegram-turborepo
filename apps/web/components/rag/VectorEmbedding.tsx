...
export function VectorEmbedding({ hasDocuments }: VectorEmbeddingProps): React.ReactElement {
  ...
  const handleGenerateEmbeddings = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setEmbeddingMessage('Successfully generated embeddings for all documents!');
      } else {
      ...
      <div className="p-6">
        ...
        {!hasDocuments ? (
          <div className="p-4 text-center rounded-lg border border-gray-600 bg-gray-700/50">
            <p className="text-gray-400">No documents available. Upload documents first to generate embeddings.</p>
          </div>
        ) : (
