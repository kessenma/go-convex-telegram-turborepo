...
export function AIChatInterface({ onOpenDocumentViewer }: AIChatInterfaceProps = {}) {
  ...
  }, [isLoading]); // Remove llmProgress methods from dependencies
  ...
  }, [error]); // Remove llmProgress.setError from dependencies

  // Custom submit handler that wraps the AI SDK's handleSubmit
                    ...
                    <AccordionContent className="pt-2 pb-1 sm:pt-3 sm:pb-2">
                      <div className="space-y-2 max-w-full">
                        {selectedDocuments.map((doc) => (
                          <div key={doc._id} className="flex gap-2 items-center p-2 min-w-0 rounded-xl border backdrop-blur-md transition-all duration-200 bg-slate-800/20 border-cyan-500/10 hover:bg-slate-700/30">
                              ...
                              <div className="relative sm:hidden">
                                ...
                                >
                                  <span className="font-medium">
                                    Powered by Vercel AI SDK + {process.env.NEXT_PUBLIC_LLM_MODEL || "Meta Llama 3.2"} LLM
                                  </span>
                                </motion.div>
                              </div>
