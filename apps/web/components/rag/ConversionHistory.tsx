...
const ConversionHistory: React.FC = () => {
  ...
  const getJobTypeColor = (jobType: string) => {
    switch (jobType) {
      case 'embedding': return 'text-purple-600 bg-purple-100';
      case 'similarity': return 'text-indigo-600 bg-indigo-100';
      case 'search': return 'text-teal-600 bg-teal-100';
      default: return 'text-gray-600 bg-gray-100';
    }
  };
    ...
    <div className="space-y-6">
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          ...
          <div className="p-4 bg-white rounded-lg shadow">
            <div className="text-2xl font-bold text-blue-600">{formatProcessingTime(stats.averageProcessingTime)}</div>
              ...
              <div className="flex justify-between items-center">
                <div className="flex-1">
                  <div className="flex items-center space-x-3">
                    ...
                    {job.llmModel && (
                      <span className="text-xs text-gray-500">{job.llmModel}</span>
                    )}
                  </div>
                  <div className="mt-2">
                    <p className="text-sm font-medium text-gray-900">Job ID: {job.jobId.slice(0, 8)}...</p>
                    {job.inputText && (
                      <p className="mt-1 text-sm text-gray-600 truncate">{job.inputText}</p>
                    )}
                  </div>
                </div>
                <div className="text-right">
                  ...
                  {job.processingTimeMs && (
                    <p className="text-sm text-gray-600">{formatProcessingTime(job.processingTimeMs)}</p>
                  )}
                </div>
              </div>
            </motion.div>
              ...
              <div className="p-6">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="font-mono text-sm text-gray-900">{selectedJob.jobId}</p>
                  </div>
                  
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Type</label>
                      <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getJobTypeColor(selectedJob.jobType)}`}>
                        {selectedJob.jobType}
                      </span>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Status</label>
                      <span className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(selectedJob.status)}`}>
                        {selectedJob.status}
                      </span>
                    </div>
                  </div>
                  ...
                  {selectedJob.inputText && (
                    <div>
                      ...
                      <p className="p-3 text-sm text-gray-900 bg-gray-50 rounded-md">{selectedJob.inputText}</p>
                    </div>
                  )}
                  
                  {selectedJob.outputData ? (
                    <div>
                      <label className="block text-sm font-medium text-gray-700">Output Data</label>
                  ...
                  {selectedJob.errorMessage && (
                    <div>
                      ...
                      <p className="p-3 text-sm text-red-600 bg-red-50 rounded-md">{selectedJob.errorMessage}</p>
                    </div>
                  )}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {selectedJob.processingTimeMs && (
                      <div>
                        ...
                        <p className="text-sm text-gray-900">{formatProcessingTime(selectedJob.processingTimeMs)}</p>
                      </div>
                    )}
                  </div>
                  ...
                  {selectedJob.llmModel && (
                    <div>
                      ...
                      <p className="text-sm text-gray-900">{selectedJob.llmModel}</p>
                    </div>
                  )}
                </div>
              </div>
