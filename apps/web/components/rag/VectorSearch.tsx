...
import { Calendar, FileText, Hash, Loader2, Search } from "lucide-react";
...
export function VectorSearch({
  className,
  hasDocuments = true,
}: VectorSearchProps): React.ReactElement {
        ...
        {!hasDocuments ? (
          <div className="p-4 text-center rounded-lg border border-gray-600 bg-gray-700/50">
            ...
            <p className="text-sm text-gray-500">
              Upload documents first to enable vector search.
            </p>
          </div>
        ) : (
          <>
            ...
            {hasSearched && results.length > 0 && (
              <div className="space-y-4">
                ...
                {results.map((result, _index) => (
                  <Card
                    key={result._id}
                    className="p-4 border-gray-600 bg-gray-700/50"
                  >
