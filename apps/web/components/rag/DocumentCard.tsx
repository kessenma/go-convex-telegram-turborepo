...
import { UploadedDocument } from '../../stores/document-store';

interface DocumentCardProps {
  document: UploadedDocument;
  isDeleting: boolean;
  expandingDocument: { docId: string; paperIndex: number } | null;
  onPaperClick: (documentId: string, paperIndex: number, event: React.MouseEvent) => void;
...
const DocumentCard = memo(function DocumentCard({
    ...
    papers.push(
      <div className="flex flex-col justify-center items-center p-2 h-full text-center">
        ...
        <div className="w-full text-xs font-medium text-gray-200 truncate">
          {doc.title.length > 12 ? doc.title.substring(0, 12) + '...' : doc.title}
        </div>
      </div>
    );
  ...
  const handlePaperClick = (paperIndex: number) => {
    ...
    const folderElement = window.document.querySelector(`[data-paper-index="${paperIndex}"]`) as HTMLElement;
    if (folderElement) {
      ...
      onPaperClick(document._id, paperIndex, mouseEvent as unknown as React.MouseEvent);
    } else {
  ...
  const handleDeleteClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    console.log('🖱️ Delete button clicked for document:', document._id, document.title);
    ...
    <div className="flex flex-col items-center space-y-3">
      <div className="relative group">
        <Folder
          ...
          expandingPaper={expandingDocument?.docId === document._id ? expandingDocument.paperIndex : null}
