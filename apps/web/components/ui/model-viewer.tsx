  ...
  useEffect(() => {
    ...
    const move = (e: PointerEvent) => {
      ...
      if (mode === "rotate") {
        ...
        e.preventDefault();
        const [p1, p2] = [...pts.values()];
        const d = p1 && p2 ? Math.hypot(p1.x - p2.x, p1.y - p2.y) : 0;
  ...
  return (
    ...
    >
      {showScreenshotButton && (
        <button
          onClick={capture}
          className="absolute top-4 right-4 z-10 px-4 py-2 text-white bg-transparent rounded-xl border border-white transition-colors cursor-pointer hover:bg-white hover:text-black"
        >
          Take Screenshot
        </button>
      )}
