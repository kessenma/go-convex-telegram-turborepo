...
function GreenContainer({ scrollProgress, isVisible, isMobile, connectionProgress }: ActivateAnimationProps) {
    ...
    return (
        React.createElement('group' as any, { ref: containerRef },
...
function BrainModel({ scrollProgress, isVisible, isMobile, connectionProgress }: ActivateAnimationProps) {
    ...
    const FallbackBrain = () => {
        ...
        useEffect(() => {
            if (mainBrainRef.current && brainStemRef.current) {
                ...
                if (brainRef.current) {
                    brainRef.current.userData.fallbackMeshes = [mainBrainRef.current, brainStemRef.current];
                }
            }
        }, []);

        return React.createElement('group' as any, {},
            // Main brain shape
            ...
            React.createElement('mesh' as any, {
                ...
                React.createElement('cylinderGeometry' as any, { args: [0.2, 0.3, 0.4, 8] }),
                React.createElement('meshPhongMaterial' as any, {
                    color: 0xcc8400,
                    transparent: true,
                    opacity: 0.8
    ...
    return (
        React.createElement('group' as any, { ref: brainRef },
            useFallback || !brainLoaded ?
                React.createElement(FallbackBrain) :
                brainObj && React.createElement('primitive', { object: brainObj.clone() })
        )
    );
}

// Connection line component
function ConnectionLine({ scrollProgress, isVisible, isMobile, connectionProgress }: ActivateAnimationProps) {
...
export function Activate({
    ...
    }, [activateVisible]); // Run when canvas becomes visible
