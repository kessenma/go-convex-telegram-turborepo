...
export function FloatingCubes({ 
  ...
  const [actualWidth, setActualWidth] = useState<number>(typeof width === 'number' ? width : (typeof window !== 'undefined' ? window.innerWidth : 400));
  ...
  useEffect(() => {
    ...
    const camera = new THREE.PerspectiveCamera(120, actualWidth / actualHeight, 0.1, 1000);
    ...
    const safeYBuffer = 2; // Buffer zone at top and bottom
  ...
  useEffect(() => {
    if ((!animationEnabled || !isInViewport) && animationIdRef.current) {
      cancelAnimationFrame(animationIdRef.current);
      animationIdRef.current = null;
    } else if (animationEnabled && isInViewport && !animationIdRef.current && sceneRef.current && rendererRef.current) {
  ...
  }, [animationEnabled, isInViewport, width, height, direction]);
