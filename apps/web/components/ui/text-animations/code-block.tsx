...
export default function CodeBlock({
  ...
  const _bgGradient = `radial-gradient(at 88% 40%,${colors.dark} 0,transparent 85%),
  ...
  return (
    <>
      <div
        className={`relative w-full max-w-2xl mx-auto rounded-xl p-0.5 ${className || ""}`}
      >
        ...
        <div className="rounded-xl bg-slate-900 p-6 shadow-[0px_-16px_24px_0px_rgba(255,255,255,0.05)_inset]">
          ...
          <pre className="overflow-x-auto p-0 m-0 text-sm leading-relaxed text-blue-100 whitespace-pre bg-transparent rounded-lg">
            <code>
              {code ? <span className="text-blue-300">{code}</span> : children}
            </code>
          </pre>
        </div>
      </div>
