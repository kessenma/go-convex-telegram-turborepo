...
import { CheckCircle, AlertCircle, Loader2, Database, Info, ChevronDown, ChevronUp, Activity, Clock, Zap } from "lucide-react";
    ...
    <div className={cn("rounded-lg border border-gray-700 bg-gray-800/50", className)}>
      <div className="flex gap-3 items-center p-3">
        <div className="flex gap-2 items-center">
          {renderIcon(Database, { className: "w-5 h-5 text-gray-400" })}
          <div className={cn(
            "rounded-full",
            statusColors[status],
            statusSizes[size],
            ...
            <div className="grid grid-cols-1 gap-2 text-xs">
              ...
              {details?.database_status && (
                <div className="flex justify-between">
                  ...
                  <span className={cn(
                    "font-mono",
                    details.database_status === 'connected' ? "text-green-400" : "text-red-400"
                  )}>
                    {details.database_status === 'connected' ? "✓ Connected" : "✗ Disconnected"}
                  </span>
                </div>
              )}
              ...
              {details?.version && (
                <div className="flex justify-between">
                  ...
                  <span className="font-mono text-gray-300">{details.version}</span>
                </div>
              )}
            </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  {statistics.requestsPerHour !== undefined && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-blue-400">{statistics.requestsPerHour}</span>
                    </div>
                  )}
                  ...
                  {statistics.requestsPerDay !== undefined && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-blue-400">{statistics.requestsPerDay}</span>
                    </div>
                  )}
                  ...
                  {statistics.successRate !== undefined && (
                    <div className="flex justify-between">
                      ...
                      <span className={cn(
                        "font-mono",
                        statistics.successRate >= 95 ? "text-green-400" : 
                        statistics.successRate >= 80 ? "text-yellow-400" : "text-red-400"
                      )}>
                  ...
                  {statistics.avgResponseTime !== undefined && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono text-purple-400">{statistics.avgResponseTime.toFixed(1)}ms</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-1 gap-2 text-xs">
                  {performance.processingTimeMs !== undefined && (
                    <div className="flex justify-between">
                      <span className="text-gray-400">Last Request Time:</span>
                      <span className="font-mono text-green-400">{performance.processingTimeMs.toFixed(1)}ms</span>
                    </div>
                  )}
                  
                  {performance.memoryUsage && performance.memoryUsage !== "N/A" && (
                    <div className="flex justify-between">
                      <span className="text-gray-400">Memory Usage:</span>
                      <span className="font-mono text-yellow-400">{performance.memoryUsage}</span>
                    </div>
                  )}
                  
                  {performance.activeConnections && performance.activeConnections !== "N/A" && (
                    <div className="flex justify-between">
                      <span className="text-gray-400">Active Connections:</span>
                      <span className="font-mono text-cyan-400">{performance.activeConnections}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {details?.last_check && (
              <div className="pt-2 border-t border-gray-700">
                <div className="flex gap-2 items-center text-xs text-gray-500">
                  ...
                  <span>Last checked: {new Date(details.last_check).toLocaleTimeString()}</span>
                </div>
              </div>
            )}
