...
const CONVEX_URL = process.env.NEXT_PUBLIC_CONVEX_URL || 'http://localhost:3211';
...
export async function POST(
  ...
  try {
    ...
    if (!vectorResponse.ok) {
      const errorBody = await vectorResponse.json().catch(() => ({ error: 'Failed to parse error response' }));
      return NextResponse.json(
    ...
    if (!saveResponse.ok) {
      const errorBody = await saveResponse.json().catch(() => ({ error: 'Failed to parse error response' }));
      return NextResponse.json(
