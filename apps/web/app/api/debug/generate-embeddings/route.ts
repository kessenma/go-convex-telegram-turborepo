...
export async function POST() {
  try {
    // Get all documents
    const documents = await convex.query(api.documents.getAllDocuments, { limit: 10 });
    ...
    for (const doc of documents.documents) {
      try {
        ...
        const existingEmbeddings = await convex.query(api.embeddings.getDocumentEmbeddings, {
          documentId: doc._id,
        });
        ...
        const result = await convex.action(api.embeddings.processDocumentWithChunking, {
          documentId: doc._id,
        });
