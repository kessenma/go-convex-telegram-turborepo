  ...
  if (messages === undefined) {
    return (
      <div className="p-6 mx-auto max-w-6xl">
        <Hero title="Telegram Messages" subtitle="Loading messages..." />
          ...
          <>
            ...
            {Object.entries(threads).map(([threadKey, threadMessages]) => {
              ...
              return (
                <Card
                  key={threadKey}
                  className="bg-gray-50 border-2 dark:bg-gray-800"
                >
                  ...
                  <div className="flex flex-col gap-3">
                    {threadMessages.map((message: TelegramMessage) => (
                      <Card
                        key={message._id}
                        className="ml-4 bg-white border-l-4 border-cyan-500 dark:bg-gray-900"
                      >
                        <div className="flex flex-wrap gap-2 justify-between items-center mb-3">
                          <span className="px-3 py-1 text-sm font-medium text-cyan-800 bg-cyan-100 rounded-full dark:bg-cyan-900 dark:text-cyan-200">
                          ...
                          {message.replyToMessageId && (
                            <span className="px-2 py-1 text-xs italic text-yellow-800 bg-yellow-100 rounded-full dark:bg-yellow-900 dark:text-yellow-200">
                        ...
                        <div className="flex justify-between items-center pt-3 text-sm border-t border-gray-100 dark:border-gray-700">
                          <span className="px-2 py-1 text-xs text-green-800 capitalize bg-green-100 rounded-full dark:bg-green-900 dark:text-green-200">
                            {message.messageType}
                          </span>
            ...
            {standaloneMessages.length > 0 && (
              <div className="mt-8">
                <h3 className="pb-2 mb-4 text-xl font-semibold text-gray-700 border-b-2 border-gray-200 dark:text-gray-300 dark:border-gray-600">
                  Standalone Messages
                </h3>
                {standaloneMessages.map((message: TelegramMessage) => (
                  <Card key={message._id}>
                    <div className="flex flex-wrap gap-2 justify-between items-center mb-3">
                      ...
                      <span className="px-3 py-1 text-sm text-gray-700 bg-gray-100 rounded-full dark:bg-gray-700 dark:text-gray-300">
                        Chat: {message.chatId}
                      </span>
                      <span className="font-mono text-sm text-gray-500 dark:text-gray-400">
                        {formatDate(message.timestamp)}
