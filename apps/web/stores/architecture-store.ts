...
export const useArchitectureStore = create<ArchitectureStore>()(
  devtools(
    (set, get) => ({
      ...
      setSecondTimelineActive: (active) =>
        set(
          (state) => ({
      ...
      setTransitionProgress: (progress) =>
        set({ transitionProgress: progress }, false, "setTransitionProgress"),
      ...
      setWhaleScrollProgress: (progress) => {
        set(
          (state) => {
            // Only update if value actually changed to prevent unnecessary re-renders
            if (Math.abs(state.whaleScrollProgress - progress) < 0.01)
              return state;
      ...
      shouldKeepSecondTimelineAlive: () => {
        const {
          ...
          secondTimelineTransitioning,
          scrollProgress,
        } = get();
        return (
          (secondTimelineActive && scrollProgress > 0) ||
          secondTimelineTransitioning
        );
      },
    }),
