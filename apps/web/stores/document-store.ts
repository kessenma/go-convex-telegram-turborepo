...
export const useDocumentStore = create<DocumentStore>()(devtools(
  (set, get) => ({
    ...
    optimisticDeleteDocument: (documentId) => {
      ...
      if (stats) {
        updatedStats = {
          ...
          totalWords: Math.max(0, stats.totalWords - documentToDelete.wordCount),
          totalSize: Math.max(0, stats.totalSize - documentToDelete.fileSize),
          contentTypes: {
            ...stats.contentTypes,
            [documentToDelete.contentType]: Math.max(0, 
              (stats.contentTypes[documentToDelete.contentType as keyof typeof stats.contentTypes] || 0) - 1
            )
          }
        };
      }
        ...
        updatedStats = {
          ...
          contentTypes: {
            ...
            [document.contentType]: 
              (stats.contentTypes[document.contentType as keyof typeof stats.contentTypes] || 0) + 1
          }
        };
      }
    ...
    deleteDocument: async (documentId) => {
      ...
      console.log('📋 Documents after optimistic update:', get().documents.length);
      ...
      try {
        ...
        console.error('❌ Document not found for deletion:', JSON.stringify(documentId));
